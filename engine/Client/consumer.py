...
service_list = Send_Broadcast_Message(
    get_list_message, get_list_addr, get_list_port, get_list
)
...
producers = Send_Broadcast_Message(
    get_request_message, get_request_addr, get_request_port, Get_request
)
print(producers)
    ...
    while True:
        ...
        if ConnectionType == "tcp":
            ...
            Thread(
                ...
                args=(ConnectionType, msg, addr, client),
                daemon=True,
            ).start()
        else:
            # hilo upd
...
def process_client_request(ConnectionType, msg, addr, client=None):
    ...
    cp = (
        socket(type=SOCK_STREAM) if ConnectionType == "tcp" else socket(type=SOCK_DGRAM)
    )
